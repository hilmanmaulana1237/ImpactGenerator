...
class AIContentController extends Controller
{
    ...
    public function index()
    {
        ...
        // Cleanup stuck jobs > 15 mins and return reserved credits when needed.
        $stuckContents = AIGeneratedContent::where('user_id', $user->id)
            ->whereIn('status', ['generating', 'queuing', 'waiting'])
            ->where('updated_at', '<', now()->subMinutes(15))
            ->get();

        foreach ($stuckContents as $content) {
            $data = json_decode($content->generated_result, true) ?? [];
            $creditType = $data['credit_type'] ?? null;

            if (in_array($creditType, ['video', 'photo_enhance'], true) && empty($data['credit_refunded'])) {
                $this->refundAICredit((int) $content->user_id, $creditType, 'Refund AI: timeout proses', $content->id);
                $data['credit_refunded'] = true;
            }

            $data['error'] = 'Timeout (System Cleanup)';

            $content->update([
                'status' => 'failed',
                'generated_result' => json_encode($data),
            ]);
        }
        ...
        $photoEnhanceCount = AIGeneratedContent::where('user_id', $user->id)
            ->where('type', 'poster')
            ->whereIn('status', ['completed', 'generating', 'queuing', 'waiting'])
            ->where(function ($query) {
                $query->where('prompt', 'like', '%"type":"enhance"%')
                    ->orWhere('prompt', 'like', '%enhance%');
            })
            ->count();
        ...
        return Inertia::render('umkm/ai-content/index', [
            ...
            'videoQuota' => [
                'used' => $videoCount,
                'max' => $videoCount + $balance->video_credits,
                'remaining' => $balance->video_credits,
            ],
            'posterQuota' => [
                'used' => $photoEnhanceCount,
                'max' => $photoEnhanceCount + $balance->photo_enhance_credits,
                'remaining' => $balance->photo_enhance_credits,
            ],
    ...
    public function generateUGCPhoto(Request $request)
    {
        try {
            ...
            if ($balance->video_credits <= 0) {
                return response()->json([
                    'success' => false,
                    'error' => 'Kuota video habis. Masukkan kupon paket AI terlebih dahulu.',
                ], 403);
            }
    ...
    public function generateVideo(Request $request)
    {
        try {
            ...
            try {
                $this->consumeAICredit($user->id, 'video', 'Generate video AI: ' . $validated['product_name']);
            ...
            if (!$result['success']) {
                if ($creditConsumed) {
                    $this->refundAICredit($user->id, 'video', 'Refund video: gagal memulai generate');
                    $creditConsumed = false;
                }
            ...
            if (!empty($creditConsumed)) {
                $this->refundAICredit(Auth::id(), 'video', 'Refund video: kesalahan sistem');
            }
    ...
    public function checkVideoStatus(Request $request)
    {
        ...
        if ($content && $content->user_id == Auth::id()) { // Use == for type coercion
            ...
            if ($result['state'] === 'success' && !empty($result['video_urls'])) {
                ...
                $existingData['error'] = $result['fail_msg'];
                if (($existingData['credit_type'] ?? null) === 'video' && empty($existingData['credit_refunded'])) {
                    $this->refundAICredit((int) $content->user_id, 'video', 'Refund video: generate gagal', $content->id);
                    $existingData['credit_refunded'] = true;
                }
    ...
    private function consumeAICredit(int $userId, string $creditType, string $description, ?string $contentId = null): void
    {
        DB::transaction(function () use ($userId, $creditType, $description, $contentId) {
            ...
            [$column, $transactionType] = $this->creditColumnAndTransactionType($creditType);

            if ($balance->{$column} <= 0) {
                throw new \RuntimeException('Kuota AI habis.');
            }
            ...
            AiCreditTransaction::create([
                ...
                'video_delta' => $creditType === 'video' ? -1 : 0,
                'photo_enhance_delta' => $creditType === 'photo_enhance' ? -1 : 0,
                'description' => $description,
                'metadata' => $contentId ? ['content_id' => $contentId] : null,
            ]);
        });
    }

    private function refundAICredit(?int $userId, string $creditType, string $description, ?string $contentId = null): void
    {
        if (! $userId) {
            return;
        }
        ...
        DB::transaction(function () use ($userId, $creditType, $description, $contentId) {
            ...
            $balance = AiCreditBalance::whereKey($balance->id)->lockForUpdate()->firstOrFail();
            [$column] = $this->creditColumnAndTransactionType($creditType);
    ...
    private function creditColumnAndTransactionType(string $creditType): array
    {
        return match ($creditType) {
            'video' => ['video_credits', AiCreditTransaction::TYPE_VIDEO_DEBIT],
            'photo_enhance' => ['photo_enhance_credits', AiCreditTransaction::TYPE_PHOTO_DEBIT],
            default => throw new \InvalidArgumentException('Unknown AI credit type.'),
        };
    }
    ...
    public function generatePosterFromTemplate(Request $request)
    {
        ...
        try {
            ...
            if ($posterType === 'enhance') {
                ...
                if ($balance->photo_enhance_credits <= 0) {
                    return response()->json([
                        'success' => false,
                        'error' => 'Kuota foto percantik habis. Masukkan kupon paket AI terlebih dahulu.',
                    ], 403);
                }

                try {
                    $this->consumeAICredit($user->id, 'photo_enhance', 'Percantik foto produk');
                } catch (\RuntimeException $e) {
                    return response()->json([
                        'success' => false,
                        'error' => 'Kuota foto percantik habis. Masukkan kupon paket AI terlebih dahulu.',
            ...
            if (!$result['success']) {
                if ($creditConsumed) {
                    $this->refundAICredit($user->id, 'photo_enhance', 'Refund foto percantik: gagal memulai generate');
                    $creditConsumed = false;
                }
            ...
            $content = AIGeneratedContent::create([
                ...
                'generated_result' => json_encode([
                    'task_id' => $result['task_id'],
                    'credit_type' => $posterType === 'enhance' ? 'photo_enhance' : null,
                ]),
            ]);
            ...
            if ($creditConsumed) {
                $this->refundAICredit(Auth::id(), 'photo_enhance', 'Refund foto percantik: kesalahan sistem');
            }
    ...
    public function checkPosterStatus(Request $request)
    {
        try {
            ...
            if ($content->status === 'completed' && $content->generated_result) {
                $completedData = json_decode($content->generated_result, true);

                return response()->json([
                    'success' => true,
                    'status' => 'completed',
                    'poster_url' => is_array($completedData)
                        ? ($completedData['poster_url'] ?? $content->generated_result)
                        : $content->generated_result,
                ]);
            }

            $existingData = json_decode($content->generated_result, true) ?? [];
            $markPosterFailed = function (string $error) use ($content, &$existingData) {
                $existingData['error'] = $error;

                if (($existingData['credit_type'] ?? null) === 'photo_enhance' && empty($existingData['credit_refunded'])) {
                    $this->refundAICredit((int) $content->user_id, 'photo_enhance', 'Refund foto percantik: generate gagal', $content->id);
                    $existingData['credit_refunded'] = true;
                }
            ...
            if (!$result['success']) {
                $markPosterFailed($result['error'] ?? 'Gagal mengecek status');
            ...
            if (($state === 'success' || $state === 'completed') && !empty($result['image_urls'])) {
                ...
                if ($savedPath) {
                    $posterUrl = route('media.generated-file', [
                        'token' => $this->encodeMediaPathToken($savedPath),
                    ]);
                    $existingData['poster_url'] = $posterUrl;
                    ...
                    $markPosterFailed('Gagal mendownload hasil poster');
                ...
                $markPosterFailed('Output image not found in response');
                ...
                $markPosterFailed($result['fail_msg'] ?? 'Generation failed');
